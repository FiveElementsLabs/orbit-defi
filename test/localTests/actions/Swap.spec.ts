...
describe('Swap.sol', function () {
  ...
  describe('Swap.sol - swap function', function () {
    it('should correctly perform a swap', async function () {
      ...
      await SwapFallback.connect(user).swap(tokenEth.address, tokenUsdc.address, 3000, amount0In, false);
    ...
    it('should revert if pool does not exist', async function () {
      ...
      await expect(SwapFallback.connect(user).swap(tokenEth.address, tokenDai.address, 2348, amount0In, false)).to.be
        .reverted;
    });
  });
});
