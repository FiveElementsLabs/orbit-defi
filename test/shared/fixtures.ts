...
import { MockToken, IUniswapV3Pool, TestRouter, Registry, Timelock, DepositRecipes } from '../../typechain';
...
export async function getPositionManager(PositionManagerFactory: any, user: Signer): Promise<Contract> {
  const tx = await PositionManagerFactory.connect(user).create();
